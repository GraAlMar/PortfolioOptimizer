...
interface ShortListTableProps {
    ...
    onDeleteFromShortList: (row: Asset) => void;
}
const ShortListTable: React.FC<ShortListTableProps> = ({assets, onSetAsMain, onDeleteFromShortList}) => {
    const columns = [
        ...
        {field: "id2",
            ...
            renderCell: row => <ThemeProvider theme={greenTheme}><Button
                onClick={() => {console.log(row); return onDeleteFromShortList(row.row)}}>Delete</Button></ThemeProvider> }
    ]
